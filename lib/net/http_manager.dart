  ...
  void _request({
    ...
    try {
      ...
      if (baseBean.errorCode == 0) {
        ...
        //失败
        String message = baseBean.errorMsg;
        var v = LogUtil.v("请求服务器出错：$message");
        if (errorCallback != null) {
          errorCallback(HttpError(baseBean.errorCode.toString(), message));
        }
      }
  ...
  Future<T> _requestAsync<T>({
    ...
    try {
      ...
      if (statusCode == 0) {
        ...
        return Future.error((HttpError(baseBean.errorCode.toString(), message)));
      }
    } on DioError catch (e, s) {
      LogUtil.v("请求出错：$e\n$s");
